...
# Project: FallenAngel
#
# Securing the web, use AI
# Powered by EduAI + FallenAngel
#
# Author: Himasnhu-at
#
# IMPROTANT: to be use for testing and
# educational Purporse. Dont use it for bad
# purposes as it's illegal'
#---------------------------------------------
...
def get_url_and_host():
    if len(sys.argv) < 2:
        raise ValueError("No URL provided as a command-line argument.")

    url = sys.argv[1]

    # Ensure that the URL has a trailing slash if it contains only two slashes (protocol and host).
    if url.count('/') == 2:
        url = url + '/'

    # Use regex to extract the host from the URL.
    ...
    print("""
      ...
      | () () |
       \  ^  /
        ...
        ||||
""")
def killnon_critical():
    for proc in psutil.process_iter(['pid', 'name', 'cpu_percent']):
        try:
            process_info = proc.info
            print(process_info)
            
            # Kill processes using minimal CPU (This is risky and just for demonstration)
